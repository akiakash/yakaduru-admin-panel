...
const TagsMultiSelect: React.FC<DropdownProps> = ({ id, onSelect }) => {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        const response = await fetchTags(); // Fetch categories from API
        ...
        if (response?.success && Array.isArray(response.data)) {
          const newOptions = response.data.map((category: any) => ({
            ...
            text: category.name, // Use name as display text
  ...
  useEffect(() => {
    onSelect(selectedValues()); // Notify parent component of selected values
  }, [selected, options]);
                ...
                >
                  <div className="flex w-full flex-col">
                    {options.map((option, index) => (
                      <div key={index}>
                        <div
                          className={`cursor-pointer border-b border-stroke bg-gray py-2 pl-4 pr-4 text-sm font-medium text-black transition hover:bg-primary hover:text-white dark:border-strokedark dark:text-white ${
                            option.selected
